...
- **Method** : RAG(기업 내부 문서) + 실시간 웹서치를 결합해 정보를 수집하고, LangGraph Supervisor 패턴으로 에이전트를 조율 (market/tech 병렬 → SWOT 순차, 결과 검증 최대 3라운드)하여 구조화된 PDF 보고서 생성
...
- LG에너지솔루션·CATL 회사별 분석을 LangGraph Send API로 병렬 실행, 기업 내 시장·기술 분석도 서브그래프로 병렬 실행
- Supervisor가 수집 결과를 체크리스트로 검증하고 부족 시 에이전트 재호출 (최대 3라운드)
...
### 에이전트
- **company_analysis_agent** (Supervisor) : LG에너지솔루션·CATL 각각에 대해 하위 에이전트를 조율하고 결과를 검증 (최대 3라운드)
  - **market_tech_agent** (서브그래프) : market_analysis_agent + tech_analysis_agent를 **병렬 실행**
    - **market_analysis_agent** : 시장 규모·점유율·원자재 가격·최신 뉴스 웹서치 수집
    - **tech_analysis_agent** : 배터리 셀 기술·공정·차세대 기술 역량 분석
  - **swot_analysis_agent** : 시장·기술 분석 결과를 바탕으로 SWOT 2×2 행렬 생성

### 노드
- **retrieve_node** : 배터리 시장 공통 배경 문서를 RAG로 선검색하여 하위 에이전트에 전달
- **company_comparison_node** : 두 기업 분석 결과를 비교하여 5개 축(기술·지역·고객·원가·신사업) 비교 보고서 작성
- **report_generation_node** : 구조화 출력으로 보고서 섹션 생성 → 참고문헌 검증 → HTML 렌더링 → PDF 저장

## Architecture

![chlwhd](https://github.com/user-attachments/assets/730687e9-951d-4d1f-9aa3-406a10ba9827)


![custom_architecture](https://github.com/user-attachments/assets/142ad267-49c1-4ab9-b08b-d18ba65ed91d)
